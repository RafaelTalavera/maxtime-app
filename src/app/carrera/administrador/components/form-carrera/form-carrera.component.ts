...
export class FormCarreraComponent {
  ...
  onSubmit(carreraForm: NgForm): void {
    if (carreraForm.valid) {
      this.carrera.organizadorId = this.organizadorId; // Asignar el organizadorId desde el input
      ...
      if (this.carrera.id && this.carrera.id > 0) {
        ...
        // Creación de carrera
        const carreraToSend = { ...this.carrera, id: undefined }; // Eliminar el id
  
        this.service.create(carreraToSend, this.selectedFiles).subscribe(
          response => {
            Swal.close(); // Cerrar el spinner
